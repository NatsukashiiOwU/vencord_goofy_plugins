...
function parseTeamMessages(messages: any[]): ParsedMessage[] {
    ...
    // Regex to capture team name (optional) and mentions, accounting for separators
    const teamRegex = /^(.*?)(?:\s*-\s*|\s+)?(<@!?\d+>.*)?$/;

    return messages
        .map(msg => {
            const content = msg.content || "";
            const matches = content.match(teamRegex);
            let teamname: string | null = null;
            let players: string[] = [];

            if (matches) {
                // Team name is the first capture group, trimmed and cleaned
                teamname = matches[1]?.trim() || null;
                if (teamname) {
                    // Remove trailing punctuation like dashes or colons
                    teamname = teamname.replace(/[-:]+$/, "").trim();
                }

                // Extract mentions from the second capture group or entire content
                const mentionContent = matches[2] || content;
                const mentionMatches = [...mentionContent.matchAll(mentionRegex)];
                players = mentionMatches.map(match => match[1]); // Extract user IDs
            }
